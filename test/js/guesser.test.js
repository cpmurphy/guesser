...
global.window = {
  ...
  TRANSLATIONS: {
    ...
    evaluation: {
      much_better:
        "Your move is even better than the game move (%{game_move})!",
      slightly_better:
        "Your move is slightly better than the game move (%{game_move}).",
...
describe("Guesser", () => {
  ...
  describe("moveBackward", () => {
    it("handles when player redoes a bad move", () => {
      ...
      expect(guesser.uiMoves[0]).toEqual({
        piece: "P",
        moves: ["e7-d8"],
        add: ["R", "d8"],
        remove: ["q", "d8"],
  ...
  describe("updateLastMoveDisplay", () => {
    ...
    it("handles a game starting with black at move 15", () => {
      ...
      guesser.moveForward();
      guesser.updateLastMoveDisplay();
      expect(document.getElementById("last-move").textContent).toBe(
        "16... Qe8",
      );
    });
  ...
  describe("initializeGuessMode", () => {
    ...
    it("sets correct guess mode when starting mid-game with black to move", () => {
      ...
      const blackRadio = document.querySelector(
        'input[name="guess_mode"][value="black"]',
      );
    ...
    it("sets correct guess mode when starting mid-game with white to move", () => {
      ...
      const whiteRadio = document.querySelector(
        'input[name="guess_mode"][value="white"]',
      );
    ...
    it("removes the captured pawn after a correct en-passant move", () => {
      const data = createGameData({
        uiMoves: [
          ...
          { piece: "P", moves: ["e4-e5"] },
          { piece: "p", moves: ["f7-f5"] },
          { piece: "P", moves: ["e5-f6"], remove: ["p", "f5"] },
        ],
      ...
      guesser.submitGuess(
        ...
        "wp",
        null,
        "rnbqkbnr/pp1pp1pp/8/2p1Pp2/8/8/PPPP1PPP/RNBQKBNR",
      );
  ...
  describe("handleGuessResponse", () => {
    ...
    it("handles guesses when the game move is a passing move", () => {
      ...
      expect(guesser.currentMoveIndex).toBe(1);
      expect(guesser.generateCompleteFen()).toBe(
  ...
  describe("navigating a whole game", () => {
    it("navigates a whole game", () => {
      ...
      expect(guesser.generateCompleteFen()).toBe(
        "r1bq1r1k/ppppnp1p/2n3p1/6B1/2PQ2N1/8/PP2PPPP/R3KB1R b KQ - 4 10",
      );
      for (let i = 0; i < 19; i++) {
        guesser.moveBackward();
      }
  ...
  describe("addExtraMove", () => {
    ...
    it("adds the extra move when not called directly", () => {
      ...
      vi.spyOn(
        guesser.evaluationExplainer,
        "explainEvaluationWithoutGameMove",
      ).mockReturnValue({
        rating: "good", // This rating...
        action: "keep_guess", // ...and this action should trigger addExtraMove.
        headline: "Mocked: Keep Guess", // Necessary for resultDisplay.update.
        comment: "Mocked evaluation specifically for testing addExtraMove.", // Necessary for resultDisplay.update.
      });

      guesser.moveForward(); // Advances currentMoveIndex, game is now "beyond" initial moves.

      // Call handleGuessResponse. The 'result' field is now less critical for triggering
      // addExtraMove because explainEvaluationWithoutGameMove is mocked.
      // The 'move' object with its 'notation' property is still essential for addExtraMove itself.
      guesser.handleGuessResponse([
        {
          result: "irrelevant_due_to_mock",
          move: { piece: "k", moves: ["d8-e8"], notation: "Ke8" },
        },
      ]);
